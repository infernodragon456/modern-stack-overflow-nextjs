  ...
  return (
    <Menubar className='relative border-none bg-transparent shadow-none'>
        <MenubarMenu  >
            ...
            <MenubarContent className='absolute right-[-3rem] mt-3 min-w-[120px] rounded border py-2 dark:border-dark-400 dark:bg-dark-300 bg-light-800'>
                {themes.map((theme) => (
                    <><MenubarItem className='flex items-center gap-4 px-2.5 py-2 dark:focus:bg-dark-400' key={theme.value} onClick={() => {
                        setMode(theme.value)
                        ...
                        <Image src={theme.icon} alt={theme.value} width={16} height={16} className={`${mode === theme.value && 'active-theme'}`}></Image>
                        <p className={`body-semibold text-light-500 ${mode === theme.value ? 'text-primary-500' : 'text-dark100_light900'}`}> {theme.label}</p>
                    </MenubarItem><MenubarSeparator></MenubarSeparator></>
                ))}
            </MenubarContent>
        </MenubarMenu>
    </Menubar>
