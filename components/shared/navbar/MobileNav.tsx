            ...
            {sidebarLinks.map((item) => {
                ...
                return (
                    ...
                    <Link href={item.route} className={`${isActive ? 'primary-gradient rounded-lg text-light-900' : 'text-dark300_light900'} flex items-center gap-4 justify-start bg-transparent p-4`}>
                        ...
                        <p className={`${isActive ? 'base-bold' : 'base-light'}`}>{item.label}</p>
                    </Link>
                </SheetClose>
                )
  ...
  return (
    <Sheet>
        <SheetTrigger asChild>
            <Image src='/assets/icons/hamburger.svg' width={36} height={36} alt='Menu' className='invert-colors sm:hidden'></Image>
        </SheetTrigger>
        <SheetContent side='left' className='background-light900_dark200 border-none'>
        <Link href='/' className='flex items-center gap-1'>
            <Image src='/assets/images/site-logo.svg' width={23} height={23} alt='Modern Stack Overflow logo'/>
            <p className='h2-bold text-dark100_light900 font-spaceGrotesk'> Modern Stack 
